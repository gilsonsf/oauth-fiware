...
public class AuthorizationCodeTokenService {
    ...
    public String getAuthorizationEndpoint(Client client) {

        Map<String, String> parametros = new HashMap<>();
        parametros.put("client_id", getEncodedUrl(client.getClientId()));
        parametros.put("redirect_uri", getEncodedUrl(client.getRedirectUri()));
        parametros.put("response_type", client.getResponseType());
        parametros.put("scope", getEncodedUrl("read write"));
        parametros.put("state", client.getState());

        return construirUrl(client.getUrlAuthorize(), parametros);
    ...
    public OAuth2Token getToken(String authorizationCode, Client client) {
...
//                URI.create("http://localhost:3005/oauth2/token")
    ...
    public void getEntity(String authorizationCode, Client client) {
        RestTemplate restTemplate = new RestTemplate();

        RequestEntity<MultiValueMap<String, String>> requestEntity = new RequestEntity<>(
                getHeader(authorizationCode),
                HttpMethod.GET,
    ...
    public static void main(String[] args) {
        new AuthorizationCodeTokenService().getEntity("", null);
    }
    ...
    private MultiValueMap<String, String> getBodyKeyrock(String authorizationCode) {
        MultiValueMap<String, String> dadosFormulario = new LinkedMultiValueMap<>();

        dadosFormulario.add("grant_type", "authorization_code");
        dadosFormulario.add("code", authorizationCode);
        dadosFormulario.add("redirect_uri",
                "http://localhost:9001/integracao/callback");
